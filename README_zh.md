...
│           │   └── lighting/                # 灯光和阴影功能模块
│           │       ├── events.cljs          # 灯光事件处理器 [已实现]
│           │       └── views.cljs           # 灯光控制组件 [已实现]
...
- ✅ 基于功能的组织：map_view、style_editor、models_3d、lighting
- ✅ 基于组件的 UI 结构，包含可重用的地图组件
- ✅ 用于外部集成的服务层（map-engine、model-loader）
- ✅ 颜色和几何的工具函数（验证逻辑已集成到服务中）
...
- ✅ 自定义 CSS 样式 UI 组件
- ✅ MapLibre CSS 集成
- ✅ 完整的 UI 组件系统（按钮、控件、布局）
- ✅ 几何工具实现
- ✅ 模型加载服务与错误处理
- ✅ 应用视图和布局组件，用于主页面结构
- ✅ 完整的 re-frame 事件和订阅系统，支持所有功能
- ✅ 地图视图，包含覆盖控件和调试信息
- ✅ 样式编辑器，支持实时建筑图层定制
- ✅ 3D 模型加载控件，包含状态指示器
- ✅ 3D 模型在地图上的渲染和放置
...
- 提供亮色和暗色主题的一键切换
- 热重载支持开发环境
- 埃菲尔铁塔 3D 模型集成（GLTF 格式） - 已加载并渲染（Three.js 渲染基础设施已就位）
...
- 完整的 UI 组件库 - 已实现
- 坐标变换的几何工具 - 已实现
- 完整的应用视图结构，包含主页布局 - 已实现
- 完整的 re-frame 架构，包含所有功能的事件、订阅和视图 - 已实现
- 地图覆盖系统，包含标题、样式控件和 3D 控件 - 已实现
...
**3D 模型状态更新：**
埃菲尔铁塔 GLTF 模型已成功加载、渲染并集成到项目结构中。Three.js 渲染基础设施已完全实现，采用本地文件集成（无 CDN 依赖）。构建过程现在自动将 Three.js 文件从 node_modules 复制到目标目录，确保 HTML 中的本地文件引用正确。模型加载机制功能正常，包含完善的错误处理，必要的构建过程确保所有资源正确部署。完整的 UI 组件系统现已实现，包含按钮、控件和布局组件。3D 模型的灯光和阴影也已实现并可控制。
...
Pearl-Map 支持三种地图样式，每种都有不同的技术实现：

**1. 栅格样式 (Raster Style)**
- **技术**: 基于OpenStreetMap栅格瓦片
- **数据源**: 德国镜像瓦片服务 (`tile.openstreetmap.de`)
- **图层**: 单一栅格图层 (`osm-tiles`)
- **特点**: 简单稳定，但不支持建筑样式编辑

**2. 矢量样式 (Vector Styles)**
- **深色样式**: Carto Dark Matter (`dark-matter-gl-style`)
- **浅色样式**: Carto Positron (`positron-gl-style`)
- **数据源**: CartoCDN矢量瓦片服务
- **图层**: 包含几十个隐式图层（道路、水域、建筑等）
- **特点**: 支持实时建筑样式编辑
...
**标准图层栈**:
```
Top:    Custom Layers     (完全自定义的WebGL渲染)
        ↓
        Building Layers   (用户可编辑的建筑样式)
        ↓
        Vector Layers     (CartoCDN提供的道路、标签等)
        ↓
Bottom: Base Layer        (栅格或矢量底图)
```
...
| 图层类型 | 数量 | 控制方式 | 功能 |
|---------|------|----------|------|
| **隐式矢量图层** | 20+ | CartoCDN自动管理 | 提供完整地图体验 |
| **建筑图层** | 2 | 用户实时编辑 | 建筑样式定制 |
| **自定义图层** | 可扩展 | 直接WebGL控制 | 3D特效和自定义渲染 |

### 建筑样式编辑器技术实现

编辑器通过**图层覆盖技术**实现建筑样式的实时编辑：

```clojure
;; 覆盖CartoCDN的默认建筑样式
:fill-color "#f0f0f0"      ; 用户可编辑的填充颜色
:fill-opacity 0.7          ; 实时透明度控制
:fill-outline-color "#cccccc" ; 轮廓颜色定制
```

**技术原理**:
- 利用CartoCDN的矢量数据源 (`composite`)
- 但使用自定义的绘制样式覆盖默认值
- 支持颜色选择器、透明度滑块的实时预览

### Custom Layer高级功能

Custom Layer提供**超越标准地图渲染的能力**：
...
```clojure
;; 栅格↔矢量切换：完全重建地图实例
;; 矢量↔矢量切换：保持地图实例，只更新样式
```

这种设计确保了不同样式类型间的平滑过渡和功能兼容性。
